...
export const performDeepAudit = async (files, registeredName) => {
    const model = genAI.getGenerativeModel({ 
        ...
        systemInstruction: "You are a Forensic Fraud Agent for HaHu Market..." 
    });
...
`;
    ...
    try {
        ...
        // This is where the audit data is parsed
        let audit = JSON.parse(jsonContent);

        // --- APPLY THE NEW ENHANCED SAFETY GATE HERE ---
        
        const isForensicallyCertain = audit.reason.includes("100% match") || audit.faceMatchScore >= 98;
        
        // 1. Check for Name Mismatch First
        if (audit.fullNameOnId.toLowerCase() !== registeredName.toLowerCase()) {
            audit.decision = "REJECT";
            audit.success = false; // Added this to be safe
            audit.reason = `IDENTITY THEFT: Name on ID (${audit.fullNameOnId}) does not match registered name (${registeredName}).`;
            return audit;
        }

        // 2. Check for Biometric Confidence
        if (isForensicallyCertain && audit.allFilesMatch) {
            audit.decision = "APPROVE";
            audit.success = true;
        } else {
            audit.decision = "REJECT";
            audit.success = false;
            // This tells you if the problem is just bad lighting vs an actual mismatch
            audit.reason = audit.faceMatchScore < 98 
                ? `Confidence too low (${audit.faceMatchScore}%). Check lighting.` 
                : audit.reason;
        }
        ...
        return { success: false, decision: "REJECT", reason: "System error during biometric analysis." };
    }
};